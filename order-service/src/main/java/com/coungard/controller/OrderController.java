...
public class OrderController {
    ...
    public ResponseEntity<List<WeightModel>> getWeightsList() {
        ...
        if (weightMap.isEmpty()) {
            return ResponseEntity.noContent().build(); // Return 204 No Content status code
        } else {
            return ResponseEntity.ok(weightMap); // Return 200 OK status code with the weightMap
        }
    }
}
